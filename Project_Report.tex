...
\subsection{Efficient Fine-Tuning with QLoRA on MLX}
To democratize access to reasoning model research, we implemented a fine-tuning pipeline for the \texttt{deepseek-math-7b-instruct} model using the Apple MLX framework.

\subsubsection{Unified Memory Architecture}
Traditional deep learning frameworks require moving data between CPU and GPU memory, creating bottlenecks. MLX leverages the Unified Memory Architecture of Apple Silicon, allowing the CPU and GPU to access the same memory pool without copying. This is critical for training large models on consumer devices.

\subsubsection{4-Bit Quantization}
We employed \textbf{QLoRA} \cite{dettmers2023qlora}, which freezes the base model parameters $W$ and injects trainable low-rank adapters $\Delta W = BA$. To minimize memory footprint, the base weights $W$ are quantized to 4-bit NormalFloat (NF4) precision.
\begin{equation}
    Y = (W_{4bit} + s \cdot BA)X
\end{equation}
This reduces the memory requirement of a 7B model from $\approx 14$GB (FP16) to $\approx 4.5$GB (INT4), enabling training on 16GB RAM devices.
